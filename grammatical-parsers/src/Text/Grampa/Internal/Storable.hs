...
instance Storable1 (ResultList g s) Bool where
   store1 bit = ResultList [] (ParseFailure (if bit then 1 else 0) [] [])
...
instance (Rank2.Functor g, Monoid s) => Storable (ResultsOfLength g s r) (ParserFlags g) where
...
instance Storable1 (Transformer.ResultListT m g s) Bool where
   store1 bit = Transformer.ResultList [] (ParseFailure (if bit then 1 else 0) [] [])
...
instance (Rank2.Functor g, Monoid s) => Storable (Transformer.ResultsOfLengthT m g s r) (ParserFlags g) where
   store = Transformer.ResultsOfLengthT . store
   reuse = reuse . Transformer.getResultsOfLength

instance (Rank2.Functor g, Monoid s) => Storable (Transformer.ResultsOfLength m g s r) (ParserFlags g) where
...
instance (Rank2.Functor g, Monoid s) => Storable [(s, g (Transformer.ResultListT m g s))] (Dependencies g) where
   store DynamicDependencies = []
   store (StaticDependencies deps) = [(mempty, store deps)]
   reuse [] = DynamicDependencies
   reuse [(_, deps)] = StaticDependencies (reuse deps)
