...
instance Alternative (Parser g i) where
   empty = Parser (\rest-> ResultList mempty $ ParseFailure (Down $ length rest) [Expected "empty"])
   Parser p <|> Parser q = Parser r where
...
instance MonoidNull s => Parsing (Parser g s) where
      ...
      where q rest = replaceFailure (p rest)
               where replaceFailure (ResultList EmptyTree (ParseFailure pos msgs)) =
                        ResultList EmptyTree (ParseFailure pos
                                              $ if pos == Down (length rest) then [Expected msg] else msgs)
   ...
   eof = Parser f
      where f rest@((s, _):_)
               ...
               | otherwise = ResultList mempty (ParseFailure (Down $ length rest) [Expected "endOfInput"])
